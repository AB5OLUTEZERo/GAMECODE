...
void UMyCharacterMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector & OldLocation, const FVector & OldVelocity)
{
	...
	if (bWantsToDodge)
	{
		...
		//Launch(DodgeVel);
		PawnOwner->LaunchPawn(DodgeVel, true, true);
		bWantsToDodge = false;
	}
	if (bWantsToJump)
	{
		...
		//FVector JumpVel = JumpDirection * JumpStrength;
		//if (IsCustomMovementMode(ECustomMovementMode::CMOVE_WallRunning))
		//{
		//	
		//	JumpVel *= WallJumpStrengthMultiplier;
		//	JumpVel += FVector(0, 0, 200);
		//}
		//GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Some debug message!"));
		////Launch(JumpVel);
		//PawnOwner->LaunchPawn(JumpVel, false, true);
		//CurrentJumpCount++;
		//
		FVector DJDir;
		if (IsCustomMovementMode(ECustomMovementMode::CMOVE_WallRunning))
		{
			FVector crossVector = WallRunSide == EWallRunSide::kLeft ? FVector(0.0f, 0.0f, 1.0f) : FVector(0.0f, 0.0f, -1.0f);
			FVector traceStart = GetPawnOwner()->GetActorLocation() + (WallRunDirection * 200);
			FVector traceEnd = (FVector::CrossProduct(WallRunDirection, crossVector) * 100);
			DJDir = traceEnd;
			
		}
		else
		{
			DJDir = FVector(0, 0, 200);
		}
		DJDir.Normalize();
		FVector JumpVel = DJDir * JumpStrength;
		if (IsCustomMovementMode(ECustomMovementMode::CMOVE_WallRunning))
			{
				
				JumpVel *= WallJumpStrengthMultiplier;
			JumpVel += FVector(0, 0, 450);
			}
		PawnOwner->LaunchPawn(JumpVel, false, true);
		CurrentJumpCount++;
		bWantsToJump = false;
...
bool UMyCharacterMovementComponent::ServerSetJumpDirection_Validate(const FVector & JumpDir)
{
	return true;
}
void UMyCharacterMovementComponent::ServerSetJumpDirection_Implementation(const FVector & JumpDir)
{
	JumpDirection = JumpDir;
}

void UMyCharacterMovementComponent::ResetJumpCount()
{
	CurrentJumpCount = 0;
}

void UMyCharacterMovementComponent::DoDoubleJump()
{
...
void UMyCharacterMovementComponent::BeginPlay()
{
	...
	if (PawnOwner->IsLocallyControlled())
	{
		...
		FVector traceEnd = FVector(0, 0, 200);
		JumpDirection = traceEnd;
	}
	if (PawnOwner->GetLocalRole() < ROLE_Authority)
	{
		ServerSetJumpDirection(JumpDirection);
	}
	ResetJumpCount();
}
...
void UMyCharacterMovementComponent::UpdateFromCompressedFlags(uint8 Flags)
{
	...
	/*  There are 4 custom move flags for us to use. Below is what each is currently being used for:
		...
		FLAG_Custom_2		= 0x40, // Dodge
		FLAG_Custom_3		= 0x80, // DoubleJump
	*/
	...
	bWantsToJump = (Flags & FSavedMove_Character::FLAG_Custom_3) != 0;
}
...
void FSavedMove_My::Clear()
{
	...
	bSavedWantsToDodge = 0;
	SavedMoveDirection = FVector::ZeroVector;
	bSavedWantsToJump = 0;
	SavedJumpDirection = FVector(0, 0, 200);
}
...
bool FSavedMove_My::CanCombineWith(const FSavedMovePtr& NewMovePtr, ACharacter* Character, float MaxDelta) const
{
	...
	if (SavedWantsToSprint != NewMove->SavedWantsToSprint ||
		...
		SavedMoveDirection != (NewMove->SavedMoveDirection)||SavedMoveDirection != (NewMove->SavedJumpDirection))
	{
		return false;
	}
...
void FSavedMove_My::SetMoveFor(ACharacter* Character, float InDeltaTime, FVector const& NewAccel, class FNetworkPredictionData_Client_Character& ClientData)
{
	...
	if (charMov)
	{
		...
		SavedJumpDirection = charMov->JumpDirection;
		bSavedWantsToJump = charMov->bWantsToJump;
	}
}
...
void FSavedMove_My::PrepMoveFor(class ACharacter* Character)
{
	...
	if (charMov)
	{
		...
		charMov->JumpDirection = SavedJumpDirection;
		charMov->bWantsToJump = bSavedWantsToJump;
	}
}
